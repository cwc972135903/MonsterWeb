...
import bgJPG from '@/assets/Logo/bz2.png'
import { closeAllTags } from '@/utils/tagsViewHelper'
import mValidate from "@/components/m-validate/m-validate";

export default {
  name: 'Login',
  // eslint-disable-next-line vue/no-unused-components
  components: { LangSelect, mValidate },
  data() {
    ...
    return {
      ...
      redirect: undefined,
      bgUrl: "url(" + bgJPG + ")",
      enableSliderValidate: window.env.VUE_SLIDER_VALIDATE == true ? true : false,
      sliderValidate: window.env.VUE_SLIDER_VALIDATE == true ? false : true //滑动校验状态
    }
  },
  watch: {
    $route: {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/login', this.loginForm)
            ...
            .catch(error => {
              this.loading = false
              this.$notify.error({ title: "登陆", message: error, duration: 2000 });
            })
        } else {
