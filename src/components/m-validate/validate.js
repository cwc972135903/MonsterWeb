 ...
 * @version: 1.0
 * @Author: wenchao.chai
 * @Date: 2020-07-27 13:17:56
 * @LastEditors: wenchao.chai
 * @LastEditTime: 2020-08-03 09:35:20
 */  
  const l = 42, // 滑块边长
    r = 9, // 滑块半径
    w = 310, // canvas宽度
  ...
  function getRandomImg () {
    return '/static/img/validate/'+Math.floor((Math.random()*10)+1)+'.jpg';
  ...
  export default class sliderValidate {
    ...
    verify () {
      ...
      const average = arr.length > 0 ? arr.reduce(sum):0 / arr.length
      const deviations = arr.map(x => x - average)
      const stddev = Math.sqrt(deviations.map(square).length>0?deviations.map(square).reduce(sum):0/ arr.length)
